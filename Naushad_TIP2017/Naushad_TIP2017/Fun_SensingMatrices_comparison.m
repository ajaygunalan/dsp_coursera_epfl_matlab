...
for k = 1:length(SampRat)                        % Iterate for different sampling ratios
    ...
    for i = 0:(NoOfBlk1-1)                         % iterate for every block in the image
        fprintf('Outerloop i is: %i\n', i);    
        for j = 0:(NoOfBlk2-1)
            fprintf('Innerloop j is: %i \n',j);      
            ...
            yb = Rb*opMatrix(img1(:));                  % compressive measurements
...
%             yg = Rg*opMatrix(img1(:));            % compressive measurements
...
%     yp = Rp*opMatrix(img1(:));            % compressive measurements
